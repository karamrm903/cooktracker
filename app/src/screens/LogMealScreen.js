...
const addMealLog =
  ...
  function getMealIcon(type) {
    return mealTypes.find(m => m.key === type)?.icon ?? 'restaurant-outline';
  }
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top', 'bottom']}>
      ...
      >
        <View style={styles.hero}>
          <Ionicons name={getMealIcon(mealType)} size={40} color="#FF6B35" />
          ...
          <View style={styles.typeRow}>
            {mealTypes.map(type => {
              ...
              return (
                ...
                >
                  <Ionicons name={type.icon} size={16} color={active ? colors.background : colors.text} />
