...
export default function CaloriesScreen({ navigation }) {
  ...
  const todayData = history[0]?.dateKey === todayKey ? history[0] : { totalCalories: 0, totalProtein: 0, totalCarbs: 0, totalFat: 0 };
  
  const calPercent = Math.min(100, Math.max(0, ((todayData.totalCalories || 0) / goalCals) * 100));
  
  const renderMacroStat = (icon: any, name: string, current: number, goal: number, color: string) => {
    const pct = Math.min(100, Math.max(0, Math.round((current / goal) * 100))) || 0;
    return (
      <View style={styles.macroStatRow}>
        ...
        <View style={styles.macroStatContent}>
          <View style={styles.macroStatHeader}>
            <Text style={[styles.macroName, { color: colors.textMuted }]}>{name}</Text>
            <Text style={styles.macroValueText}>
              <Text style={[styles.macroCurrent, { color: colors.text }]}>{Math.round(current)}g </Text>
              <Text style={[styles.macroGoal, { color: colors.textMuted }]}>/{goal}g</Text>
            </Text>
          </View>
          <View style={styles.macroStatBarHeader}>
            <View style={[styles.macroMiniBarBg, { backgroundColor: colors.surfaceAlt }]}>
              <View style={[styles.macroMiniBarFill, { width: `${pct}%`, backgroundColor: color }]} />
            </View>
  ...
  const renderHeader = () => {
    ...
    const remainingText = remaining >= 0 ? `${remaining} kcal remaining` : `${Math.abs(remaining)} kcal over`;
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.titleSection}>
          ...
          <View style={styles.dateHeader}>
            ...
            <Image source={calendarImg} style={styles.calendarIcon} resizeMode="contain" />
          </View>
        </View>
          ...
          <View style={styles.summaryTop}>
            <View style={styles.progressRingContainer}>
              ...
              <View style={styles.ringTextContainer}>
                <Image source={fireImg} style={styles.ringFireIcon} resizeMode="contain" />
                <Text style={[styles.ringKcalText, { color: colors.text }]}>{Math.round(todayData.totalCalories)}</Text>
                <Text style={[styles.ringSubText, { color: colors.textMuted }]}>/{goalCals} kcal</Text>
              </View>
            </View>
            ...
            <View style={styles.macrosColumn}>
              ...
              {renderMacroStat(muscleIcon, 'Protein', todayData.totalProtein, goalProtein, MACRO_COLORS.protein)}
              {renderMacroStat(leafIcon, 'Carbs', todayData.totalCarbs, goalCarbs, MACRO_COLORS.carbs)}
              {renderMacroStat(dropIcon, 'Fat', todayData.totalFat, goalFat, MACRO_COLORS.fat)}
            </View>
          </View>
        </View>
      </View>
  ...
  if (loading && page === 1) {
    return (
      <SafeAreaView style={[styles.safe, styles.loaderCenter, { backgroundColor: SCREEN_BG }]}>
        <ActivityIndicator size="large" color={ORANGE_CAL} />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: SCREEN_BG }]} edges={['top']}>
      <FlatList
        ...
        renderItem={({ item }) => <ExpandableDayCard dayData={item} goalCals={goalCals} />}
        ListHeaderComponent={renderHeader}
        showsVerticalScrollIndicator={false}
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[ORANGE_CAL]} tintColor={ORANGE_CAL} />
        }
        onEndReached={loadMore}
        onEndReachedThreshold={0.5}
        ListFooterComponent={
          hasMore && history.length > 0 ? (
            <ActivityIndicator size="small" color={ORANGE_CAL} style={{ marginVertical: ms(20) }} />
          ) : (
...
const styles = StyleSheet.create({
  ...
  listContent: { paddingHorizontal: SPACING.md, paddingTop: ms(10), paddingBottom: ms(40) },
